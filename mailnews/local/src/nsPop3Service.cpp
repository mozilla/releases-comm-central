...
nsresult nsPop3Service::GetMail(bool downloadNewMail, nsIMsgWindow* aMsgWindow,
  ...
  if (destLocalFolder) {
    ...
    destLocalFolder->WarnIfLocalFileTooBig(aMsgWindow, 0xFFFF,
                                           &destFolderTooBig);
    if (destFolderTooBig) return NS_MSG_ERROR_WRITING_MAIL_FOLDER;
  }
  ...
  if (popHost.IsEmpty()) return NS_MSG_INVALID_OR_MISSING_SERVER;
    ...
    char* urlSpec =
        (downloadNewMail)
            ? PR_smprintf("pop3://%s@%s:%d", escapedUsername.get(),
                          popHost.get(), popPort)
            : PR_smprintf("pop3://%s@%s:%d/?check", escapedUsername.get(),
                          popHost.get(), popPort);
...
void nsPop3Service::AlertServerBusy(nsIMsgMailNewsUrl* url) {
  ...
  rv = url->GetMsgWindow(getter_AddRefs(
      msgWindow));  // it is ok to have null msgWindow, for example when biffing
  if (NS_FAILED(rv) || !msgWindow) return;
...
nsPop3Service::GetDefaultLocalPath(nsIFile** aResult) {
  ...
  *aResult = nullptr;
  ...
  nsresult rv = NS_GetPersistentFile(PREF_MAIL_ROOT_POP3_REL,
                                     PREF_MAIL_ROOT_POP3, NS_APP_MAIL_50_DIR,
  ...
  if (!havePref || !exists) {
    rv = NS_SetPersistentFile(PREF_MAIL_ROOT_POP3_REL, PREF_MAIL_ROOT_POP3,
                              localFile);
