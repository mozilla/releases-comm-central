...
class CharPtrArrayGuard {
 public:
  explicit CharPtrArrayGuard(bool freeElements = true)
      : mFreeElements(freeElements), mArray(0), mSize(0) {}

  ~CharPtrArrayGuard() { Free(); }

  char* operator[](int i) { return mArray[i]; }

  uint32_t* GetSizeAddr(void) { return &mSize; }

  uint32_t GetSize(void) { return mSize; }

  char*** GetArrayAddr(void) { return &mArray; }

  const char** GetArray(void) { return (const char**)mArray; }
...
class PRUnicharPtrArrayGuard {
 public:
  ...
  ~PRUnicharPtrArrayGuard() { Free(); }

  char16_t* operator[](int i) { return mArray[i]; }

  uint32_t* GetSizeAddr(void) { return &mSize; }

  uint32_t GetSize(void) { return mSize; }

  char16_t*** GetArrayAddr(void) { return &mArray; }

  const char16_t** GetArray(void) { return (const char16_t**)mArray; }
