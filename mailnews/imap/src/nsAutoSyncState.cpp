...
NS_IMETHODIMP nsAutoSyncState::ProcessExistingHeaders(
  ...
  *aLeftToProcess = keyCount - mProcessPointer;
  ...
  if (0 == *aLeftToProcess) {
    ...
    // Handle the case where imap STATUS triggered an update but nothing found
    // to download when folder SELECTed.
    if ((mSyncState != stCompletedIdle) && mDownloadQ.IsEmpty()) {
      nsCString folderName;
      folder->GetURI(folderName);
      MOZ_LOG(gAutoSyncLog, LogLevel::Debug,
              ("%s: nothing to download for folder %s, "
               "set state to stCompletedIdle",
               __func__, folderName.get()));
      SetState(stCompletedIdle);
      // Set autosync manager back to completed state.
      nsCOMPtr<nsIAutoSyncManager> autoSyncMgr =
          do_GetService(NS_AUTOSYNCMANAGER_CONTRACTID, &rv);
      NS_ENSURE_SUCCESS(rv, rv);
      ...
      NS_ENSURE_SUCCESS(rv, rv);
      autoSyncMgrListener->OnStopRunningUrl(nullptr, NS_OK);
    }
  }
  return rv;
}
...
NS_IMETHODIMP nsAutoSyncState::OnStopRunningUrl(nsIURI* aUrl,
  ...
  if (mSyncState == stStatusIssued) {
    ...
    if (serverNextUID != mLastNextUID || serverTotal != mLastServerTotal ||
        ...
        ((serverUnseen != mLastServerUnseen) && mLastServerUnseen > -1)) {
    ...
    } else  // folderstatus detected no change
    {
      ...
      // now empty database so changes above are compared to folder cache. If
      // so, force an update which will re-populate the database (.msf) and
      // download all the message to mbox/maildir store. This check is only
      // needed after the first imap STATUS response after start-up. Note: A
      // restart is required when .msf (the folder database) and mbox/maildir
      // storage are manually deleted in filesystem by the user.
      if (!mHaveAStatusResponse) {
        nsCOMPtr<nsIMsgDatabase> database;
      ...
      if (mSyncState == stStatusIssued) {
        ...
        rv = autoSyncMgrListener->OnStopRunningUrl(nullptr, NS_OK);
      }
