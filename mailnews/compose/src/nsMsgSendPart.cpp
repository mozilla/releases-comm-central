...
nsMsgSendPart::~nsMsgSendPart() {
  for (int i = 0; i < m_numchildren; i++) delete m_children[i];
...
nsresult nsMsgSendPart::SetMimeDeliveryState(nsIMsgSend* state) {
  ...
  if (GetNumChildren() > 0) {
    for (int i = 0; i < GetNumChildren(); i++) {
      ...
      if (part) part->SetMimeDeliveryState(state);
    }
  }
  return NS_OK;
}
...
nsresult nsMsgSendPart::AppendOtherHeaders(const char* more) {
  ...
  if (!more || !*more) return NS_OK;

  char* tmp = (char*)PR_Malloc(sizeof(char) *
                               (PL_strlen(m_other) + PL_strlen(more) + 2));
...
nsMsgSendPart* nsMsgSendPart::DetachChild(int32_t whichOne) {
  ...
  NS_ASSERTION(whichOne >= 0 && whichOne < m_numchildren,
  ...
  if (returnValue) returnValue->m_parent = nullptr;
...
"Content-".)
  ...
  */
static nsresult divide_content_headers(const char* headers,
  ...
  int L = 0;
  if (headers) L = PL_strlen(headers);

  if (L == 0) return NS_OK;

  *message_headers = (char*)PR_Malloc(L + 1);
  if (!*message_headers) return NS_ERROR_OUT_OF_MEMORY;
  ...
  *type_tail = 0;
  ...
  if (*content_type_header) {
    ...
    if (tmp) {
      ...
      NS_ASSERTION(!PL_strstr(tmp, "Content-Type"),
                   "Content-part already present");
    }
  }
  ...
  if (*content_headers) {
    ...
    if (tmp) {
      tmp++;  // get past the first occurrence
      NS_ASSERTION(!PL_strstr(tmp, "Content-Transfer-Encoding"),
                   "Content-Transfert already present");
    }
  }
#endif  // DEBUG
...
#define PUSH(str) PUSHLEN(str, PL_strlen(str))
  ...
  if ((m_parent) && (m_numchildren == 0) && ((!m_buffer) || (!*m_buffer)) &&
      (!m_file) && (!m_mainpart))
  ...
  if (m_mainpart && m_type && PL_strcmp(m_type, TEXT_HTML) == 0) {
    if (m_file) {
      ...
      int32_t length = 0;
      int64_t fileSize;
      if (NS_SUCCEEDED(m_file->GetFileSize(&fileSize))) length = fileSize;

      m_buffer = (char*)PR_Malloc(sizeof(char) * (length + 1));
      if (m_buffer) {
        nsCOMPtr<nsIInputStream> inputFile;
        nsresult rv =
            NS_NewLocalFileInputStream(getter_AddRefs(inputFile), m_file);
        if (NS_SUCCEEDED(rv)) {
          uint32_t bytesRead;
  ...
  if (m_parent && m_parent->m_type &&
      !PL_strcasecmp(m_parent->m_type, MULTIPART_DIGEST) && m_type &&
    ...
    status = divide_content_headers(m_other, &message_headers, &content_headers,
                                    &content_type_header);
    if (NS_FAILED(status)) goto FAIL;
    ...
    if (!content_type_header) {
      ...
      if (needsCharset) {
        ...
        content_type_header = PR_smprintf("Content-Type: %s" CRLF,
                                          (m_type ? m_type : TEXT_PLAIN));
      if (!content_type_header) {
        if (content_headers) PR_Free(content_headers);
    ...
    */
    if (m_numchildren > 0) {
      ...
      if (content_type_header[L - 1] == '\r') content_type_header[--L] = 0;

      ct2 = PR_smprintf("%s;\r\n boundary=\"%s\"" CRLF, content_type_header,
                        separator);
      PR_Free(content_type_header);
      if (!ct2) {
        if (content_headers) PR_Free(content_headers);
  ...
  if (m_buffer) {
    ...
    nsresult rv =
        NS_NewLocalFileInputStream(getter_AddRefs(inputStream), m_file);
    if (NS_FAILED(rv)) {
    ...
    if (m_strip_sensitive_headers) {
      ...
      while (more) {
        ...
        if (!PL_strncasecmp(line, "From -", 6) ||
            ...
            !PL_strncasecmp(line, X_MOZILLA_STATUS2 ":",
                            X_MOZILLA_STATUS2_LEN + 1) ||
            !PL_strncasecmp(line, X_MOZILLA_DRAFT_INFO ":",
                            X_MOZILLA_DRAFT_INFO_LEN + 1) ||
        ...
        if (curLine.Length() == 2) {
          ...
          seekableStream->Seek(nsISeekableStream::NS_SEEK_CUR,
                               lineBuffer->start - lineBuffer->end);
    ...
    while (NS_SUCCEEDED(status)) {
      ...
      if (NS_FAILED(status)) goto FAIL;
      if (bytesRead < MIME_BUFFER_SIZE) break;
    }
  }
