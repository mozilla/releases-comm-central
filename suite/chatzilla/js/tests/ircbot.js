...
function init(obj) {
  ...
  obj.networks["libera.chat"] = new CIRCNetwork(
    "libera.chat",
    [{ name: "libera.chat", port: 6667 }],
    obj.eventPump
  );

  obj.networks.efnet = new CIRCNetwork(
    "efnet",
    [
...
bot.personality.addHook = function psn_addhook(pattern, f, name, neg, enabled) {
  if (pattern instanceof RegExp) {
    pattern = { statement: pattern };
  }

  return bot.eventPump.addHook(
    pattern,
    f,
    name,
    neg,
...
function my_chan_privmsg(e) {
  ...
  if (msg.indexOf(bot.prefix) == 0 && userIsOwner(user)) {
    ...
    if (msg[msg.length - 1] == "\\") {
      user.accumulatedScript = msg.substring(bot.prefix.length, msg.length - 1);
      return false; // prevent other hooks from processing this...
    }

    return bot_eval(e, msg.substring(bot.prefix.length, msg.length));
  } else if (
    typeof user.accumulatedScript != "undefined" &&
    userIsOwner(user)
  ) {
