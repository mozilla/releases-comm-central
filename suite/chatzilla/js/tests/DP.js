 ...
 */
CDPressMachine.prototype.addPhrase = function DPM_addPhrase(strPhrase, weight) {
  if (strPhrase == "") {
    return;
  }
  ...
  for (var i = -1; i < aryWordMatches.length; i++) {
    ...
    if (typeof currentWordPivot == "undefined") {
      currentWordPivot = this.wordPivots[currentWord] = new CWordPivot(
        currentWord
      );
    }
...
CDPressMachine.prototype.addPhrases = function DPM_addPhrases(phrases) {
 ...
 */
CDPressMachine.prototype.getPhraseDirected = function DPM_getPhraseDirected(
  seedWord,
  dir
) {
  ...
  do {
    ...
    if (word != "") {
      ...
      if (c > CDPressMachine.MAX_PHRASE_LENGTH) {
        if (
          (dir > 0 && this.wordPivots[word].nextList.list[""]) ||
          (dir <= 0 && this.wordPivots[word].previousList.list[""])
        ) {
      ...
      if (
        c < CDPressMachine.MIN_PHRASE_LENGTH &&
...
CDPressMachine.prototype.getPhraseForward = function DPM_getPhraseForward(
  firstWord
) {
  return this.getPhraseDirected(firstWord, 1);
};

CDPressMachine.prototype.getPhraseReverse = function DPM_getPhraseReverse(
  lastWord
) {
 ...
 * links from |word|.
 */
CDPressMachine.prototype.getRandomPivot = function DPM_getRandomPivot(word) {
  /**
  ...
  if (false) {
    var depth = parseInt(
      Math.round(CDPressMachine.RANDOMIZE_DEPTH * Math.random())
    );
    word = "";
    for (
      var i = 0;
      i < depth, (word = this.wordPivots[word].nextList.getRandomLink().link);
      i++
    ) {} /* empty loop */
  }
};

CDPressMachine.prototype.getPhrase = function DPM_getPhrase(word) {
 ...
 */
CDPressMachine.prototype.getPhraseContaining = function DPM_getPhraseContaining(
  word
) {
  ...
  var rval, spc;
  var post,
    pre = this.getPhraseReverse(word);
  if (word != "") {
    var post = this.getPhraseForward(word);
  }

  dpprint(
    "* DPM_getPhraseContaining: pre = '" + pre + "' post = '" + post + "'"
  );
...
CDPressMachine.prototype.getPhraseWeight = function DPM_getPhraseWeight(
  phrase
) {
...
CDPressMachine.prototype.getPhraseWeights = function DPM_getPhraseWeights(
  phrase
) {
  var words,
    ary = new Array();
...
CDPressMachine.prototype.getPivot = function DPM_getPivot(word) {
  return this.wordPivots[word];
};

CDPressMachine.prototype.trimList = function DPM_trimList(threshold) {
  var el;
  var c;
...
CDPressMachine.prototype.getMachineStatus = function DPM_getMachineStatus() {
  var o = new Object();
...
CPhraseLinkList.prototype.addLink = function PLL_addLink(link, weight) {
  ...
  } else if (!(typeof existingLink.adjust == "function")) {
    dd(
      "existingLink.adjust is a '" +
        existingLink.adjust +
        "' " +
        "not a function! link is '" +
        link +
        "'"
    );
  } else {
...
CPhraseLinkList.prototype.getRandomLink = function PLL_getRandomLink() {
  var tot = 0;
  var lastMatch = "";
...
CPhraseLinkList.prototype.getListWeights = function PLL_getListWeights() {
  var ary = new Array();
...
CPhraseLinkList.prototype.getListLinks = function PLL_getListLinks() {
  var ary = new Array();
...
CPhraseLinkList.prototype.trimList = function PLL_trimList(threshold) {
  var el;
  var c;
...
CPhraseLink.prototype.adjust = function PL_adjust(weight) {
  if ((this.weight += weight) < 1) {
    this.weight = 1;
  }
};

CPhraseLink.prototype.weight = function PL_weight() {
  return this.weight;
};
