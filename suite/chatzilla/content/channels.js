...
const colIDToSortKey = {
  chanColName: "name",
  chanColUsers: "users",
  chanColTopic: "topic",
};
const sortKeyToColID = {
  name: "chanColName",
  users: "chanColUsers",
  topic: "chanColTopic",
};
...
function onLoad() {
  function ondblclick(event) {
    tree.view.onRouteDblClick(event);
  }
  function onkeypress(event) {
    tree.view.onRouteKeyPress(event);
  }
  function onfocus(event) {
    tree.view.onRouteFocus(event);
  }
  function onblur(event) {
    tree.view.onRouteBlur(event);
  }
...
function update() {
  ...
  if (network) {
    ...
    xul.lastUpdated.value = listMod
      ? getMsg(MSG_CD_UPDATED, [
          strftime(MSG_CD_UPDATED_FORMAT, new Date(listMod)),
        ])
...
function startOperation(op) {
  ...
  if (
    !ASSERT(
      opData.state == STATE_IDLE || ops.canStop,
...
function processOpLoadStart(opData) {
  ...
  if (tree.view.childData.childData.length > 1) {
    tree.view.childData.removeChildrenAtIndex(
      1,
      tree.view.childData.childData.length - 1
    );
  }
...
function processOpFilterRun(opData) {
  ...
  updateProgress(
    getMsg(MSG_CD_FILTERING, [opData.currentIndex, channels.length]),
