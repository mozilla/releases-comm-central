...
add_setup(async () => {
  ...
  await createSubfolder(gLocalRootFolder, "Sent");
  await createSubfolder(gLocalRootFolder, "Templates");
...
async function runTest(mainConfig) {
  const files = {
    "background.js": async () => {
  ...
  extension.onMessage("getConfig", async () => {
    extension.sendMessage(mainConfig);
  });
