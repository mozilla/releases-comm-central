...
const subtest_clickInBrowser = async (extension, getBrowser) => {
      ...
      PERMISSION_DIALOG_URL,
      {
        ...
        callback(dialogWin) {
          is(
            ...
            PERMISSION_DIALOG_URL,
            "Open dialog is the permission dialog"
          );

          const dialog = dialogWin.document.querySelector("dialog");
          const btn = dialog.getButton("accept");
  ...
  for (const expectedSelector of testSelectors) {
    ...
    // This resolves window.sendMessage("click", { linkSelector }) which got us
    // here.
