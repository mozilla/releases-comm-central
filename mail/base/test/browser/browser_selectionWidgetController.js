 ...
 * @param {string="right-to-left"} options.direction - The direction of the
...
add_task(async function test_empty_widget_focus() {
  ...
  for (let shiftKey of [false, true]) {
    for (let ctrlKey of [false, true]) {
      info(`Clicking empty widget: ctrlKey: ${ctrlKey}, shiftKey: ${shiftKey}`);
...
add_task(function test_keyboard_navigation() {
  for (let { direction, forwardKey, backwardKey } of [
    ...
    info(`Keyboard navigation ${direction}`);
    reset({ direction });
    widget.addItems(0, ["First", "Second", "Third"]);
    ...
    assertSingleSelection(0, "First item remains selected on Ctrl+Backward");
    ...
    for (let key of [forwardKey, backwardKey, "KEY_Home", "KEY_End"]) {
      for (let ctrlKey of [false, true]) {
        info(`Pressing ${ctrlKey ? "Ctrl+" : ""}Shift+${key} in ${direction}`);
        EventUtils.synthesizeKey(key, { shiftKey: true, ctrlKey }, win);
        assertFocus(widget.items[1].element, "Focus still on second item");
        assertSingleSelection(0, "Selection still on first item");
      }
    }
  }
});
...
add_task(function test_space_selection() {
  ...
  for (let { shiftKey, ctrlKey } of [
    ...
    info(`Pressing space on item: ctrlKey: ${ctrlKey}, shiftKey: ${shiftKey}`);
    // On selected item.
...
add_task(function test_clicking_items() {
  ...
  for (let { shiftKey, ctrlKey } of [
    ...
    assertFocus(widget.items[1].element, "Widget item is refocused");
    assertSingleSelection(1, "Selected remains on second item");
...
add_task(function test_add_items_to_nonempty() {
  ...
  assertOrder(["2-add", "3-add", "4-add", "5-add", "0-add", "6-add", "1-add"]);
...
add_task(function test_empty_widget() {
