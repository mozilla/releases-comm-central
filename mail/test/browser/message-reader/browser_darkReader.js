...
async function assert_light_style() {
  ...
  if (AppConstants.DEBUG || AppConstants.ASAN || AppConstants.TSAN) {
    await new Promise(resolve => aboutMessage.requestIdleCallback(resolve));
    await new Promise(resolve => aboutMessage.requestAnimationFrame(resolve));
  }
  await SpecialPowers.spawn(
    ...
    function () {
      ...
      const headStyle = content.getComputedStyle(
      ...
      const headStyle2 = content.getComputedStyle(
      ...
      const headStyle3 = content.getComputedStyle(
      ...
      const tableBlock = content.getComputedStyle(
...
async function assert_dark_style() {
  ...
  await SpecialPowers.spawn(
    ...
    function () {
      ...
      const style = content.getComputedStyle(
        msgDoc.querySelector("#headStyle")
      );
      Assert.equal(
        style.color,
