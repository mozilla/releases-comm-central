...
add_task(async function test_no_messages_selected() {
  await be_in_folder(folder);
...
add_task(async function test_one_message_selected() {
  await be_in_folder(folder);
...
add_task(async function test_n_messages_selected() {
  await be_in_folder(folder);
...
add_task(async function test_between_tab_and_single_message() {
  await be_in_folder(folder);
  select_click_row(0);
  let tab = await open_selected_message_in_new_tab(true);
...
add_task(async function test_between_tab_and_multi_message() {
  await be_in_folder(folder);
  select_click_row(0);
  let tab = await open_selected_message_in_new_tab(true);
...
add_task(async function test_after_delete() {
  await be_in_folder(folder);
  make_display_threaded();
  collapse_all_threads();
