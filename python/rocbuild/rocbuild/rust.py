...
# it is generated by comm/python/rocbuild/rocbuild/rust.py
[package]
name = "gkrust"
version = "0.1.0"
edition = "2024"
...
def get_cargo(command_context):
    """
    Ensure cargo is installed and version is new enough.
...
class CargoFile:
    ...
    def _handle_workspace(self, data):
        ...
        if "package" in data and "rust-version" in data["package"]:
            self.rust_version = data["package"]["rust-version"]
...
def cleanup_vendor(command_context):
    ...
    for package_dir in vendored_dir.iterdir():
...
def regen_toml_files(command_context, workspace):
    ...
    rust_version = mc_workspace.rust_version
