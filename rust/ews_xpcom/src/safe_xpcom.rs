...
impl<L: RefCounted + 'static> SafeListenerWrapper<L> {
    /// Return a new wrapper for the given listener. Will place the given borrow
    /// into a [`RefPtr`] to ensure the memory management of the inner callbacks
    /// is done correctly across the XPCOM boundary and guarantee that the
    /// lifetime of the borrowed inner listener meets or exceeds the lifetime of
    /// the returned wrapper for use in asynchronous operations.
