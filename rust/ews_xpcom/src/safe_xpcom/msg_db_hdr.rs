...
/// populated. This type only exposes safe methods for populating fields, which
/// turns the underlying type into a [`PopulatedMsgDbHeader`].
pub struct UnpopulatedMsgDbHeader(RefPtr<nsIMsgDBHdr>);

/// Wrapper newtype for an [`nsIMsgDBHdr`] that is only created by populating
/// the fields of an [`UnpopulatedMsgDbHeader`]. Because of this,
/// `PopulatedMsgDbHeader` (and its unwrapped version) is safe to use anywhere
/// that assumes a populated `nsIMsgDBHdr`.
pub struct PopulatedMsgDbHeader(RefPtr<nsIMsgDBHdr>);

impl UnpopulatedMsgDbHeader {
    ...
    pub fn populate_from_message_headers(
    ...
    ) -> Result<PopulatedMsgDbHeader, nsresult> {
        ...
        Ok(PopulatedMsgDbHeader(self.0))
    }
...
impl From<PopulatedMsgDbHeader> for RefPtr<nsIMsgDBHdr> {
    fn from(hdr: PopulatedMsgDbHeader) -> Self {
        hdr.0
    }
}
...
impl From<RefPtr<nsIMsgDBHdr>> for UnpopulatedMsgDbHeader {
    fn from(hdr: RefPtr<nsIMsgDBHdr>) -> Self {
        UnpopulatedMsgDbHeader(hdr)
    }
}
