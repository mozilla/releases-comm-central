...
pub enum UserExpand {
    ...
    SponsorOf(ExpandOptions<DirectoryObjectSelection>),
...
impl fmt::Display for UserExpand {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            UserExpand::SponsorOf(opt) => opt.full_format(f, ExpandNames::from(self)),
...
pub struct User {
    ...
    #[doc = "true if the account is enabled; otherwise, false.\n\n This property is required when a user is created. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, and `in`). This property is subject to sensitive action restrictions; only specific privileged administrator roles can update it."]
    pub account_enabled: Option<Nullable<bool>>,
    #[doc = "Sets the age group of the user.\n\n Allowed values: `null`, `Minor`, `NotAdult`, and `Adult`. For more information, see legal age group property definitions. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, and `in`)."]
    pub age_group: Option<Nullable<String>>,
    #[doc = "The birthday of the user.\n\n The Timestamp type represents date and time information using ISO 8601 format and is always in UTC. For example, midnight UTC on Jan 1, 2014, is 2014-01-01T00:00:00Z. Requires `$select` to retrieve."]
    pub birthday: Option<String>,
    #[doc = "The telephone numbers for the user.\n\n NOTE: Although it's a string collection, only one number can be set for this property. Read-only for users synced from the on-premises directory. Returned by default. Supports `$filter` (`eq`, `not`, `ge`, `le`, `startsWith`). This property is subject to sensitive action restrictions; only specific privileged administrator roles can update it."]
    ...
    #[doc = "The city where the user is located.\n\n Maximum length is 128 characters. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    pub city: Option<Nullable<String>>,
    #[doc = "The name of the company that the user is associated with.\n\n This property can be useful for describing the company that a guest comes from. The maximum length is 64 characters.Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    pub company_name: Option<Nullable<String>>,
    #[doc = "Sets whether consent was obtained for minors.\n\n Allowed values: `null`, `Granted`, `Denied`, and `NotRequired`. For more information, see legal age group property definitions. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, and `in`)."]
    ...
    #[doc = "Indicates whether the user account was created through one of the following methods:  As a regular school or work account (null).\n\n As an external account (Invitation). As a local account for an Azure Active Directory B2C tenant (LocalAccount). Through self-service sign-up by an internal user using email verification (EmailVerified). Through self-service sign-up by a guest signing up through a link that is part of a user flow (SelfServiceSignUp). Read-only.Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `in`)."]
    pub creation_type: Option<Nullable<String>>,
    #[doc = "The name of the department in which the user works.\n\n Maximum length is 64 characters. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, and `eq` on null values)."]
    ...
    #[doc = "Captures enterprise worker type.\n\n For example, Employee, Contractor, Consultant, or Vendor. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`)."]
    ...
    #[doc = "The time when this Microsoft Entra user last changed their password or when their password was created, whichever date the latest action was performed.\n\n The date and time information uses ISO 8601 format and is always in UTC. For example, midnight UTC on Jan 1, 2014 is 2014-01-01T00:00:00Z. Requires `$select` to retrieve."]
    ...
    #[doc = "The mail alias for the user.\n\n This property must be specified when a user is created. Maximum length is 64 characters. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    pub mail_nickname: Option<Nullable<String>>,
    #[doc = "Settings for the primary mailbox of the signed-in user.\n\n You can get or update settings for sending automatic replies to incoming messages, locale, and time zone. Requires `$select` to retrieve."]
    ...
    #[doc = "The primary cellular telephone number for the user.\n\n Read-only for users synced from the on-premises directory. Maximum length is 64 characters. Returned by default. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values) and `$search`. This property is subject to sensitive action restrictions; only specific privileged administrator roles can update it."]
    ...
    #[doc = "Contains the on-premises Active Directory distinguished name or DN.\n\n The property is only populated for customers who are synchronizing their on-premises directory to Microsoft Entra ID via Microsoft Entra Connect. Read-only. Requires `$select` to retrieve."]
    ...
    #[doc = "Specifies password policies for the user.\n\n This value is an enumeration with one possible value being DisableStrongPassword, which allows weaker passwords than the default policy to be specified. DisablePasswordExpiration can also be specified. The two might be specified together; for example: `DisablePasswordExpiration, DisableStrongPassword`. Requires `$select` to retrieve. For more information on the default password policies, see Microsoft Entra password policies. Supports `$filter` (`ne`, `not`, and `eq` on null values)."]
    ...
    #[doc = "The postal code for the user's postal address.\n\n The postal code is specific to the user's country or region. In the United States of America, this attribute contains the ZIP code. Maximum length is 40 characters. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    ...
    #[doc = "The preferred name for the user.\n\n Not Supported. This attribute returns an empty string.Requires `$select` to retrieve."]
    ...
    #[doc = "Any refresh tokens or session tokens (session cookies) issued before this time are invalid.\n\n Applications get an error when using an invalid refresh or session token to acquire a delegated access token (to access APIs such as Microsoft Graph). If this happens, the application needs to acquire a new refresh token by requesting the authorized endpoint. Read-only. Use revokeSignInSessions to reset. Requires `$select` to retrieve."]
    pub sign_in_sessions_valid_from_date_time: Option<Nullable<String>>,
    #[doc = "A list for the user to enumerate their skills.\n\n Requires `$select` to retrieve."]
    pub skills: Option<Vec<String>>,
    #[doc = "Directory objects that this user sponsors, such as guest users, agent users, agent blueprints, agent blueprint principals, and agent identities.\n\n If the user is a member of a group that's a sponsor, the objects sponsored by that group are also included. Read-only. Nullable. Supports `$filter`, $count, `$select`, `$expand`, `$top`, and $skip."]
    pub sponsor_of: Option<Vec<DirectoryObject>>,
    ...
    #[doc = "The street address of the user's place of business.\n\n Maximum length is 1,024 characters. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    ...
    #[doc = "A two-letter country code (ISO standard 3166).\n\n Required for users that are assigned licenses due to legal requirements to check for availability of services in countries/regions. Examples include: US, JP, and GB. Not nullable. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `ge`, `le`, `in`, `startsWith`, and `eq` on null values)."]
    ...
    #[doc = "A string value that can be used to classify user types in your directory.\n\n The possible values are Member and Guest. Requires `$select` to retrieve. Supports `$filter` (`eq`, `ne`, `not`, `in`, and `eq` on null values). NOTE: For more information about the permissions for members and guests, see What are the default user permissions in Microsoft Entra ID?"]
