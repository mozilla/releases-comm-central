...
// Wrap in a block to prevent leaking to window scope.
{
    ...
    class CalendarTimeBar extends MozXULElement {
        ...
        setDayStartEndHours(dayStartHour, dayEndHour) {
            if (dayStartHour * 60 < this.mStartMin ||
        ...
        relayout() {
            ...
            this.getSections().forEach(([startMinute, duration]) => {
                ...
                box.setAttribute("class",
                    "calendar-time-bar-box-" + (hour % 2 == 0 ? "even" : "odd"));
    ...
    class CalendarMultidayBaseView extends MozElements.CalendarBaseView {
        ...
        connectedCallback() {
            ...
            this.addEventListener("scroll", (event) => {
                ...
                if (scrollbox.scrollHeight > 0) {
                    const scrollTopOrLeft = scrollbox.getAttribute("orient") == "horizontal"
                        ? scrollbox.scrollTop
        ...
        updateTimeIndicatorPosition(updateTheTimer, ppmChanged, viewChanged) {
            ...
            if (updateTheTimer) {
                ...
                const prefInt = ppmChanged && this.mPixPerMin < 0.6
            ...
            const todayColumn = this.findColumnForDate(this.today());
            if (todayColumn) {
                todayColumn.column.timeIndicatorBox
                    .setAttribute("style", "margin-" + posAttr + position + "px;");
            }
        }
        ...
        handlePreference(subject, topic, preference) {
            ...
            switch (preference) {
                case "calendar.view.daystarthour":
                    this.setDayStartEndMinutes(subject.getIntPref(preference) * 60,
                        this.mDayEndMin);
                    this.refreshView();
                    break;
                ...
                case "calendar.view.dayendhour":
                    this.setDayStartEndMinutes(this.mDayStartMin,
                        subject.getIntPref(preference) * 60);
                    this.refreshView();
                    break;
        ...
        onResize(calViewElem) {
            ...
            const ppm = ppmRounded < self.mMinPixelsPerMinute
                ? self.mMinPixelsPerMinute
                : ppmRounded;
        ...
        relayout() {
            ...
            const setUpDayEventsBox = (dayBox, date) => {
                dayBox.setAttribute("class", "calendar-event-column-" + (counter % 2 == 0 ? "even" : "odd"));
            ...
            for (const date of computedDateList) {
                ...
                switch (date.compare(today)) {
                    ...
                    case 0: {
                        ...
                        setBooleanAttribute(dayEventsBox.timeIndicatorBox, "hidden",
        ...
        findColumnsForOccurrences(occurrences) {
            ...
            for (const occ of occurrences) {
                const startDate = occ[cal.dtz.startDateProp(occ)]
                    .getInTimezone(this.mStartDate.timezone);

                const endDate = occ[cal.dtz.endDateProp(occ)]
        ...
        findColumnsForItem(item) {
            ...
            let finishDate = (item.endDate || item.dueDate || item.entryDate || startDate)
        ...
        deleteItemsFromCalendar(calendar) {
            ...
            for (const col of this.mDateColumns) {
                ...
                const colEvents = col.header.mItemBoxes.map(box => box.occurrence)
        ...
        scrollToMinute(rawMinute) {
            ...
            const maxFirstMin = 25 * 60 - Math.round(
                scrollbox.getBoundingClientRect().height / this.mPixPerMin);
        ...
        setDayStartEndMinutes(dayStartMin, dayEndMin) {
            ...
            if (dayStartMin < this.mStartMin ||
                dayStartMin > dayEndMin ||
                dayEndMin > this.mEndMin) {
                throw Cr.NS_ERROR_INVALID_ARG;
            }
            if (this.mDayStartMin != dayStartMin ||
