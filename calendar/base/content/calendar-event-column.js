    ...
    class MozCalendarEventColumn extends MozXULElement {
        ...
        clear() {
            ...
            for (let handler of this.mHandlersToRemove) {
                document.getElementById("calendarviewBroadcaster").removeEventListener(
                    this.calendarView.getAttribute("type") + "viewresized", handler, true);
            }
            this.mHandlersToRemove = [];
            this.mSelectedChunks = [];
        }
        ...
        onEventSweepMouseMove(event) {
            ...
            if (event.clientX < (boundingRect.x) ||
                ...
                for (let column = firstCol, i = firstIndex; column && i < col.mDragState.shadows; column = column.nextSibling, i++) {
                    column.fgboxes.dragbox.removeAttribute("dragging");
                    column.fgboxes.box.removeAttribute("dragging");
                }
        ...
        startSweepingToModifyEvent(aEventBox, aOccurrence, aGrabbedElement, aMouseX, aMouseY, aSnapInt) {
        ...
        setDayStartEndMinutes(dayStartMin, dayEndMin) {
            if (dayStartMin < this.mStartMin || dayStartMin > dayEndMin ||
