...
def verify_docs(filename, identifiers, appearing_as):
    ...
    for expression, identifier in zip(expression_list, identifiers):
        ...
        if not match_group:
            raise Exception(f"{appearing_as}: `{identifier}` missing from doc file: `{filename}`")


# TODO: Fix failures caught by this verification and enable it
    ...
    for route in routes:
        if route.startswith(route_prefix):
            ...
            if route_filter not in valid_filters:
                raise Exception(f"{task.label} has invalid notification filter ({route_filter})")
...
def verify_test_packaging(task, taskgraph, scratch_pad, graph_config, parameters):
    if task is None:
        ...
        if test_env.get("MOZHARNESS_TEST_PATHS", "") or test_env.get("MOZHARNESS_TEST_TAG", ""):
...
def task_matcher_exception_generator(
    ...
    if expected_group and group_symbol:
        if label_contains in task_label and expected_group not in group_symbol:
            raise Exception(
        ...
        if label_contains in task_label and expected_symbol not in symbol:
            raise Exception(
            ...
            if "enterprise" in task.label:
                ...
                if "win64" in task.label:
                    if "-msi-" in task.label:
                        ...
                        if "moz_sbxTb" in task.label:
                            task_matcher_exception_generator(
                                "repacks MSI",
                                task.label,
                                "repackage-enterprise-repack-msi-",
                                symbol=symbol,
                                expected_symbol="moz/sbxTb",
                            )
                            task_matcher_exception_generator(
                                "repacks MSI signed",
                                task.label,
