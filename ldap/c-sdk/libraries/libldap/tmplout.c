...
#if defined(_WINDOWS) || defined(aix) || defined(SCOOS) || defined(OSF1) || \
    defined(SOLARIS)
...
#define NONFATAL_LDAP_ERR(err)                              \
  (err == LDAP_SUCCESS || err == LDAP_TIMELIMIT_EXCEEDED || \
...
static int do_entry2text(LDAP* ld, char* buf, /* NULL for use-internal */
  ...
  if (writeproc == NULL || !NSLDAPI_VALID_LDAPMESSAGE_ENTRY_POINTER(entry)) {
  ...
  if (html) {
    ...
    if (!((opts & LDAP_DISP_OPT_HTMLBODYONLY) != 0)) {
      ...
      sprintf(buf, "%s</TITLE>%s</HEAD>%s<BODY>%s<H3>%s - ", eol, eol, eol, eol,
              (tmpl == NULL) ? "Entry" : tmpl->dt_name);
      (*writeproc)(writeparm, buf, strlen(buf));
      output_dn(buf, dn, 0, rdncount, writeproc, writeparm, "", NULL);
    ...
    if ((opts & LDAP_DISP_OPT_NONLEAF) != 0 &&
      ...
      sprintf(buf + strlen(buf), "%s\">Move Up To <EM>%s</EM></A>%s<BR>",
              (vals[1] == NULL) ? "??one" : "",
  ...
  if (tmpl == NULL) {
    ...
    for (attr = ldap_first_attribute(ld, entry, &ber);
      ...
      err =
          do_vals2text(ld, buf, vals, attr, labelwidth, LDAP_SYN_CASEIGNORESTR,
    ...
    for (rowp = ldap_first_tmplrow(tmpl);
      ...
      for (colp = ldap_first_tmplcol(tmpl, rowp); colp != NULLTMPLITEM;
        ...
        if (colp->ti_attrname == NULL ||
          ...
          freevals = 0;
          if (!LDAP_IS_TMPLITEM_OPTION_SET(colp, LDAP_DITEM_OPT_HIDEIFEMPTY) &&
              defattrs != NULL && colp->ti_attrname != NULL) {
            for (i = 0; defattrs[i] != NULL; ++i) {
              if (strcasecmp(colp->ti_attrname, defattrs[i]) == 0) {
                break;
              }
            }
            if (defattrs[i] != NULL) {
          ...
          freevals = 1;
          if (LDAP_IS_TMPLITEM_OPTION_SET(colp, LDAP_DITEM_OPT_SORTVALUES) &&
        ...
        if (colp->ti_syntaxid == LDAP_SYN_SEARCHACTION) {
          if ((opts & LDAP_DISP_OPT_DOSEARCHACTIONS) != 0) {
            if (colp->ti_attrname == NULL ||
              ...
              err =
                  searchaction(ld, buf, base, entry, dn, colp, labelwidth,
                               rdncount, writeproc, writeparm, eol, urlprefix);
            }
          }
          show = 0;
        }
        ...
        if (show) {
          err = do_vals2text(ld, buf, vals, colp->ti_label, labelwidth,
                             colp->ti_syntaxid, writeproc, writeparm, eol,
                             rdncount, urlprefix);
        }
...
static int do_entry2text_search(
...
#ifdef CLDAP
      if (LDAP_IS_CLDAP(ld))
        err = cldap_search_s(ld, dn, LDAP_SCOPE_BASE, "objectClass=*", ocattrs,
                             0, &ldmp, NULL);
      else
#endif /* CLDAP */
        err = ldap_search_st(ld, dn, LDAP_SCOPE_BASE, "objectClass=*", ocattrs,
                             0, &timeout, &ldmp);
...
#ifdef CLDAP
  if (LDAP_IS_CLDAP(ld))
    err = cldap_search_s(ld, dn, LDAP_SCOPE_BASE, "objectClass=*", fetchattrs,
                         0, &ldmp, NULL);
  else
#endif /* CLDAP */
    err = ldap_search_st(ld, dn, LDAP_SCOPE_BASE, "objectClass=*", fetchattrs,
                         0, &timeout, &ldmp);
  ...
  if (err != LDAP_SUCCESS || (entry = ldap_first_entry(ld, ldmp)) == NULL) {
    NSLDAPI_FREE(buf);
    return (LDAP_GET_LDERRNO(ld, NULL, NULL));
  }

  err = do_entry2text(ld, buf, base, entry, tmpl, defattrs, defvals, writeproc,
                      writeparm, eol, rdncount, opts, urlprefix);
...
static int do_vals2text(LDAP* ld, char* buf, /* NULL for "use internal" */
  ...
  for (i = 0; vals[i] != NULL; ++i) {
    ...
    notascii = (*p != '\0');
    outval = notascii ? "(unable to display non-ASCII text value)" : vals[i];
    ...
    switch (syntaxid) {
      ...
      case LDAP_SYN_DN: /* for now */
        output_dn(buf, outval, labelwidth, rdncount, writeproc, writeparm, eol,
                  urlprefix);
        break;
...
/* CTIME for this platform doesn't use this. */
  ...
  t.tm_mon = GET2BYTENUM(p) - 1;
  p += 2;
  t.tm_mday = GET2BYTENUM(p);
  p += 2;
  t.tm_hour = GET2BYTENUM(p);
  p += 2;
  t.tm_min = GET2BYTENUM(p);
  p += 2;
  t.tm_sec = GET2BYTENUM(p);
  p += 2;
...
#if !defined(macintosh) && !defined(_WINDOWS) && !defined(DOS) && \
    !defined(XP_OS2)
#  include <sys/time.h>
#endif /* !macintosh */
...
static long gtime(struct tm* tm) {
  ...
  if ((sec = tm->tm_sec) < 0 || sec > 59 || (mins = tm->tm_min) < 0 ||
      mins > 59 || (hour = tm->tm_hour) < 0 || hour > 24 ||
      (mday = tm->tm_mday) < 1 || mday > 31 || (mon = tm->tm_mon + 1) < 1 ||
      mon > 12)
    return ((long)-1);
  if (hour == 24) {
    hour = 0;
  ...
  result = 0L;
  for (i = 1970; i < year; i++) result += dysize(i);
  if (dysize(year) == 366 && mon >= 3) result++;
  while (--mon) result += dmsize[mon - 1];
  result += mday - 1;
...
static int searchaction(LDAP* ld, char* buf, char* base, LDAPMessage* entry,
  ...
  if (html) {
    ...
    sprintf(buf + strlen(buf), "\"><B>%s</B></A><DD><BR>%s", tip->ti_label,
            eol);
...
#ifdef CLDAP
  if (LDAP_IS_CLDAP(ld))
    lderr = cldap_search_s(ld, base, LDAP_SCOPE_SUBTREE, filter, retattrs, 0,
                           &ldmp, NULL);
  else
#endif /* CLDAP */
    lderr = ldap_search_st(ld, base, LDAP_SCOPE_SUBTREE, filter, retattrs, 0,
                           &timeout, &ldmp);
  ...
  if (lderr == LDAP_SUCCESS || NONFATAL_LDAP_ERR(lderr)) {
    if ((count = ldap_count_entries(ld, ldmp)) > 0) {
      if ((members = (char**)NSLDAPI_MALLOC((count + 1) * sizeof(char*))) ==
        ...
        for (i = 0, entry = ldap_first_entry(ld, ldmp); entry != NULL;
        ...
        err = do_vals2text(ld, NULL, members, tip->ti_label, html ? -1 : 0,
