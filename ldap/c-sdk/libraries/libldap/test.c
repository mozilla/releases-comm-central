  ...
  if (!freeit) {
    ...
    *dnp = dn;
    ...
    if (*authmethodp == LDAP_AUTH_SIMPLE && dn[0] != '\0') {
      getline(passwd, sizeof(passwd), stdin, "re-bind password? ");
    } else {
      passwd[0] = '\0';
    }
    *passwdp = passwd;
 ...
 int argc, char **argv )
{
...
#ifdef TEST_CUSTOM_MALLOC
  {
    ...
    if (ldap_set_option(NULL, LDAP_OPT_MEMALLOC_FN_PTRS, &memalloc_fns) != 0) {
...
#  endif
      ...
      case 'T': /* only output ber's to given file */
        copyfname = strdup(optarg);
        copyoptions =
            (LBER_SOCKBUF_OPT_TO_FILE | LBER_SOCKBUF_OPT_TO_FILE_ONLY);
        break;
#endif
...
#  endif
  ...
  while (getline(line, sizeof(line), stdin, "\ncommand? ") != NULL) {
    ...
    switch (command1) {
      case 'a': /* add or abandon */
        switch (command2) {
          case 'd': /* add */
            ...
            strcat(dn, dnsuffix);
            if ((attrs = get_modlist(NULL, "attr? ", "value? ")) == NULL) break;
      ...
      case 'b': /* asynch bind */
        getline(line, sizeof(line), stdin, "method 0->simple 3->sasl? ");
        method = atoi(line);
        if (method == 0) {
        ...
        if (method == LDAP_AUTH_SIMPLE) {
          if (dn[0] != '\0') {
            getline(passwd, sizeof(passwd), stdin, "password? ");
          } else {
            passwd[0] = '\0';
          }
          ...
          if (ldap_sasl_bind(ld, dn, mechanism, &cred, NULL, NULL, &rc) !=
              LDAP_SUCCESS) {
            rc = -1;
          }
        }
      ...
      case 'x': /* extended operation */
      {
        ...
        if (ldap_extended_operation(ld, oid, &val, NULL, NULL, &id) !=
            LDAP_SUCCESS) {
      ...
      case 'C': /* set cache parameters */
#ifdef NO_LIBLCACHE
        getline(line, sizeof(line), stdin, "cache init (memcache 0)? ");
#else
        ...
        if (i == 0) { /* memcache */
          ...
          ttl = atoi(line);
          getline(line, sizeof(line), stdin, "memcache size? ");
          size = atoi(line);
          getline(line, sizeof(line), stdin, "memcache baseDN? ");
          if (*line == '\0') {
            basedns = NULL;
          } else {
            dnarray[0] = line;
          ...
          if ((rc = ldap_memcache_init(ttl, size, basedns, NULL, &mc)) !=
              LDAP_SUCCESS) {
            fprintf(stderr, "ldap_memcache_init: %s\n", ldap_err2string(rc));
          } else if ((rc = ldap_memcache_set(ld, mc)) != LDAP_SUCCESS) {
            fprintf(stderr, "ldap_memcache_set: %s\n", ldap_err2string(rc));
          }

#ifndef NO_LIBLCACHE
        } else if (i == 1) {
          getline(line, sizeof(line), stdin, "cache config file? ");
          if (line[0] != '\0') {
          ...
          getline(line, sizeof(line), stdin, "cache on/off (on 1, off 0)? ");
          if (line[0] != '\0') {
            i = atoi(line);
            if (ldap_set_option(ld, LDAP_OPT_CACHE_ENABLE, &i) != 0) {
          ...
          if (line[0] != '\0') {
            i = atoi(line);
            if (ldap_set_option(ld, LDAP_OPT_CACHE_STRATEGY, &i) != 0) {
...
#  endif
      ...
      case 'm': /* modify or modifyrdn */
        if (strncmp(line, "modify", 4) == 0) {
          ...
          getline(line, sizeof(line), stdin, "deleteoldrdn? ");
          if ((id = ldap_modrdn2(ld, dn, rdn, atoi(line))) == -1)
      ...
      case 'q': /* quit */
#ifdef CLDAP
        if (cldapflg) cldap_close(ld);
#endif /* CLDAP */
        if (!cldapflg) ldap_unbind(ld);
        exit(0);
        break;
      ...
      case 'r': /* result or remove */
        switch (command3) {
          case 's': /* result */
            getline(line, sizeof(line), stdin, "msgid (-1=>any)? ");
            if (line[0] == '\0')
              id = -1;
            else
              id = atoi(line);
            getline(line, sizeof(line), stdin, "all (0=>any, 1=>all)? ");
            if (line[0] == '\0')
              all = 1;
            else
              all = atoi(line);
            if ((msgtype = ldap_result(ld, id, all, tvp, &res)) < 1) {
      ...
      case 's': /* search */
        ...
        if (cldapflg) {
#ifdef CLDAP
          getline(line, sizeof(line), stdin, "Requestor DN (for logging)? ");
          if (cldap_search_s(ld, dn, scope, filter, types, attrsonly, &res,
                             line) != 0) {
          ...
          if ((id = ldap_search(ld, dn, scope, filter, types, attrsonly)) ==
              -1) {
      ...
      case 'u': /* user friendly search w/optional timeout */
        ...
        if (command2 == 't') {
          id = ldap_ufn_search_c(ld, dn, types, attrsonly, &res,
                                 ldap_ufn_timeout, &timeout);
        } else {
          id = ldap_ufn_search_s(ld, dn, types, attrsonly, &res);
        }
        if (res == NULL)
      ...
      case 'l': /* URL search */
        ...
        if ((id = ldap_url_search(ld, line, attrsonly)) == -1) {
      ...
      case 'p': /* parse LDAP URL */
        ...
        if ((i = ldap_url_parse(line, &ludp)) != 0) {
          ...
          printf("\tsecure: %s\n",
                 (ludp->lud_options & LDAP_URL_OPT_SECURE) != 0 ? "Yes" : "No");
          printf("\t    dn: ");
          if (ludp->lud_dn == NULL) {
            printf("ROOT\n");
          } else {
      ...
      case 'N': /* add an LDAPv3 control */
        getline(line, sizeof(line), stdin, "Control oid (. to clear list)? ");
        if (*line == '.' && *(line + 1) == '\0') {
          controls = NULL;
        } else {
          ...
          getline(line, sizeof(line), stdin, "Critical (0=no, 1=yes)? ");
          newctrl->ldctl_iscritical = atoi(line);
          add_control(&controls, newctrl);
        }
        ldap_set_option(ld, LDAP_OPT_SERVER_CONTROLS, controls);
        ldap_get_option(ld, LDAP_OPT_SERVER_CONTROLS, &tmpctrls);
      ...
      case 'P': /* add a persistent search control */
        ...
        getline(line, sizeof(line), stdin, "Critical (0=no, 1=yes)? ");
        if (ldap_create_persistentsearch_control(
                ld, changetypes, changesonly, return_echg_ctls,
                (char)atoi(line), &newctrl) != LDAP_SUCCESS) {
          ldap_perror(ld,
                      "ldap_create_persistent"
                      "search_control");
        } else {
          add_control(&controls, newctrl);
          ldap_set_option(ld, LDAP_OPT_SERVER_CONTROLS, controls);
...
#  endif /* LDAP_CHARSET_8859 */
        ...
        optval = (atoi(line) != 0);
        if (optval) {
          getline(line, sizeof(line), stdin, "security DB path?");
          if (ldapssl_client_init((*line == '\0') ? NULL : line, NULL) < 0) {
            perror("ldapssl_client_init");
            optval = 0; /* SSL not avail. */
          } else if (ldapssl_install_routines(ld) < 0) {
            ldap_perror(ld, "ldapssl_install_routines");
            optval = 0; /* SSL not avail. */
          }
        }

        ldap_set_option(ld, LDAP_OPT_SSL, optval ? LDAP_OPT_ON : LDAP_OPT_OFF);

        getline(line, sizeof(line), stdin, "Set SSL options (0=no, 1=yes)?");
        optval = (atoi(line) != 0);
        while (1) {
          PRInt32 sslopt;
          PRBool on;

          getline(line, sizeof(line), stdin, "Option to set (0 if done)?");
          sslopt = atoi(line);
          if (sslopt == 0) {
            break;
          }
          getline(line, sizeof(line), stdin, "On=1, Off=0?");
          on = (atoi(line) != 0);
        ...
        ldap_set_option(ld, LDAP_OPT_RECONNECT,
                        (atoi(line) == 0) ? LDAP_OPT_OFF : LDAP_OPT_ON);
      ...
      case 'I': /* initialize display templates */
        ...
        if ((i = ldap_init_templates(
                 *line == '\0' ? "ldaptemplates.conf" : line, &tmpllist)) !=
            0) {
      ...
      case 'T': /* read & display using template */
        ...
        strcat(dn, dnsuffix);
        if ((i = ldap_entry2text_search(ld, dn, NULL, NULL, tmpllist, NULL,
                                        NULL, entry2textwrite, stdout, "\n", 0,
                                        0)) != LDAP_SUCCESS) {
      ...
      case 'L': /* set preferred language */
        getline(line, sizeof(line), stdin, "Preferred language? ");
        if (*line == '\0') {
          ldap_set_option(ld, LDAP_OPT_PREFERRED_LANGUAGE, NULL);
        } else {
          ldap_set_option(ld, LDAP_OPT_PREFERRED_LANGUAGE, line);
        }
        break;
      ...
      case 'F': /* create filter */
      {
        ...
        getline(pattern, sizeof(pattern), stdin, "pattern? ");
        getline(prefix, sizeof(prefix), stdin, "prefix? ");
        getline(suffix, sizeof(suffix), stdin, "suffix? ");
        getline(attr, sizeof(attr), stdin, "attribute? ");
        getline(value, sizeof(value), stdin, "value? ");
        ...
        if (ldap_create_filter(filtbuf, sizeof(filtbuf), pattern, prefix,
                               suffix, attr, value, words) != 0) {
          fprintf(stderr, "ldap_create_filter failed\n");
        } else {
          printf("filter is \"%s\"\n", filtbuf);
        }
...
static void print_ldap_result(LDAP* ld, LDAPMessage* lm, char* s) {
  ...
  if (ldap_msgtype(lm) == LDAP_RES_BIND &&
      ldap_parse_sasl_bind_result(ld, lm, &servercred, 0) == LDAP_SUCCESS &&
      servercred != NULL) {
  ...
  if (ldap_msgtype(lm) == LDAP_RES_EXTENDED &&
      ldap_parse_extended_result(ld, lm, &oid, &data, 0) == LDAP_SUCCESS) {
    if (oid != NULL) {
      if (strcmp(oid, LDAP_NOTICE_OF_DISCONNECTION) == 0) {
        printf("\t%s Notice of Disconnection (OID: %s)\n", s, oid);
      } else {
        printf("\t%s OID: %s\n", s, oid);
      }
      ldap_memfree(oid);
...
static void print_search_entry(LDAP* ld, LDAPMessage* res, int onlyone) {
  ...
  for (msg = ldap_first_message(ld, res);
      ...
      if ((vals = ldap_get_values_len(ld, e, a)) == NULL) {
        ...
        for (i = 0; vals[i] != NULL; i++) {
          ...
          if (nonascii) {
            ...
            bprint(vals[i]->bv_val, vals[i]->bv_len);
    ...
    if (ldap_get_lderrno(ld, NULL, NULL) != LDAP_SUCCESS) {
      ldap_perror(ld, "ldap_first_attribute/ldap_next_attribute");
    }
    if (ber != NULL) {
      ber_free(ber, 0);
    }

    if (ldap_get_entry_controls(ld, e, &ectrls) != LDAP_SUCCESS) {
...
static void print_search_reference(LDAP* ld, LDAPMessage* res, int onlyone) {
  ...
  for (msg = ldap_first_message(ld, res);
    ...
    if (ldap_parse_reference(ld, msg, &refs, &ctrls, 0) != LDAP_SUCCESS) {
...
static void print_controls(LDAPControl** ctrls, int freeit) {
  ...
  for (i = 0; ctrls[i] != NULL; ++i) {
    ...
    fputs("\tvalue:\n", stderr);
    bprint(ctrls[i]->ldctl_value.bv_val, ctrls[i]->ldctl_value.bv_len);
  }
