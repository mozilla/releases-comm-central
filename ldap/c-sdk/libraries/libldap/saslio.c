...
typedef struct lextiof_socket_private {
  struct ldap_x_ext_io_fns sess_io_fns; /* the saved layered ld fns from the
  ...
  while (dlen < 4) {
    if (arg->sock_io_fns.lbextiofn_read != NULL) {
      ret = arg->sock_io_fns.lbextiofn_read(
          s, cp, 4 - dlen, arg->sock_io_fns.lbextiofn_socket_arg);
    } else {
      ret = read(s, cp, 4 - dlen);
    }
#  ifdef EINTR
    if ((ret < 0) && (LDAP_GET_ERRNO(ld) == EINTR)) continue;
#  endif
    if (ret <= 0) return (ret);
  ...
  while (dlen > 0) {
    if (arg->sock_io_fns.lbextiofn_read != NULL) {
      ret = arg->sock_io_fns.lbextiofn_read(
          s, cp, dlen, arg->sock_io_fns.lbextiofn_socket_arg);
    } else {
      ret = read(s, cp, dlen);
    }
...
#  endif
  ...
  /* Decode the packet */
  ret = sasl_decode(arg->sasl_ctx, arg->sb_sasl_ibuf, blen, &dbuf, &dlen);
  if (ret != SASL_OK) {
    ...
    while (olen > 0) {
      if (arg->sock_io_fns.lbextiofn_write != NULL) {
        ret = arg->sock_io_fns.lbextiofn_write(
            s, optr, olen, arg->sock_io_fns.lbextiofn_socket_arg);
      } else {
 ...
 * First, take a look at os-ip.c:nsldapi_add_to_cb_pollfds().  When a new
 * descriptor is added to the pollfds array, the lpoll_socketarg field is
 * initialized to the value from the socketarg field -
 * sb->sb_ext_io_fns.lbextiofn_socket_arg.  In our case, since we override this
 * with our sasl data (see below nsldapi_sasl_install), we need to restore the
 * previous value so that the layer below us (i.e. prldap) can use the
 * lpoll_socketarg which it sets. So how do which know which fds[i] is a "sasl"
 * fd? We initialize the lextiof_session_private *arg (see nsldapi_sasl_install)
 * to point to the socket_private data in sb->sb_ext_io_fns.lbextiofn_socket_arg
 * for "sasl" sockets, which is then used to initialize lpoll_socketarg (see
 * above). So, if the arg which gets passed into nsldapi_sasl_poll is the same
 * as the fds[i].lpoll_socketarg, we know it is a "sasl" socket and we need to
 * "pop" the sasl layer.  We do this by replacing lpoll_socketarg with the one
 * we saved when we "pushed" the sasl layer. So why the loop to restore the sasl
 * lpoll_socketarg? The lower layer only uses lpoll_socketarg during poll(). See
 * ldappr-io.c:prldap_poll() for more information about how that works. However,
 * after the polling is done, there is some special magic in os-ip.c in the
 * functions nsldapi_add_to_cb_pollfds(), nsldapi_clear_from_cb_pollfds(), and
 * nsldapi_find_in_cb_pollfds() to find the correct Sockbuf to operate on.  This
 * is the macro NSLDAPI_CB_POLL_MATCH().  For the extended io function callbacks
 * to work correctly, it is not sufficient to say that the file descriptor in
 * the Sockbuf matches the one that poll says has activity - we also need to
 * match the lpoll_socketarg with the sb->sb_ext_io_fns.lbextiofn_socket_arg to
 * make sure this really is the Sockbuf we want to use.  So we have to restore
 * the lpoll_socketarg with the original one. Why have origarg and
 * staticorigarg? To avoid malloc.  The sizeof staticorigarg should be large
 * enough to accommodate almost all clients without incurring too much
 * additional overhead.  However, if we need more room, origarg will grow to
 * nfds.  If this proves to be inadequate, the size of the staticorigarg is a
...
int nsldapi_sasl_open(LDAP* ld, LDAPConn* lconn, sasl_conn_t** ctx,
  ...
  saslrc = sasl_client_new("ldap", host, NULL,
                           NULL, /* iplocalport, ipremoteport - use defaults */
                           NULL, 0, ctx);
  ldap_memfree(host);
...
int nsldapi_sasl_install(LDAP* ld, LDAPConn* lconn) {
  ...
  rc = ldap_get_option(ld, LDAP_X_OPT_EXTIO_FN_PTRS, &sockarg->sess_io_fns);
  if (rc != 0) {
    destroy_SASLIOSocketArg(&sockarg);
    return (LDAP_LOCAL_ERROR);
  }
