...
int LDAP_CALL ldap_modify_ext(LDAP* ld, const char* dn, LDAPMod** mods,
  ...
  *msgidp = ++ld->ld_msgid;
  ...
  if (ld->ld_cache_on && ld->ld_cache_modify != NULL) {
    ...
    if ((rc = (ld->ld_cache_modify)(ld, *msgidp, LDAP_REQ_MODIFY, dn, mods)) !=
        0) {
      *msgidp = rc;
      LDAP_MUTEX_LOCK(ld, LDAP_CACHE_LOCK);
      return (LDAP_SUCCESS);
  ...
  if (ber_printf(ber, "{it{s{", *msgidp, LDAP_REQ_MODIFY, dn) == -1) {
  ...
  for (i = 0; mods[i] != NULL; i++) {
    if ((mods[i]->mod_op & LDAP_MOD_BVALUES) != 0) {
      rc = ber_printf(ber, "{e{s[V]}}", mods[i]->mod_op & ~LDAP_MOD_BVALUES,
                      mods[i]->mod_type, mods[i]->mod_bvalues);
    } else {
      rc = ber_printf(ber, "{e{s[v]}}", mods[i]->mod_op, mods[i]->mod_type,
                      mods[i]->mod_values);
    }
