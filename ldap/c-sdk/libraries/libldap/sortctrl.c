...
int LDAP_CALL ldap_create_sort_control(LDAP* ld, LDAPsortkey** sortKeyList,
  ...
  /* the sort control value will be encoded as a sequence of sequences
     which are each encoded as one of the following: {s} or {sts} or {stb} or
     {ststb} since the orderingRule and reverseOrder flag are both optional */
  for (i = 0; sortKeyList[i] != NULL; i++) {
    /* encode the attributeType into the ber */
    if (ber_printf(ber, "{s", (sortKeyList[i])->sk_attrtype) == -1) {
      goto encoding_error_exit;
    }
    ...
    if ((sortKeyList[i])->sk_matchruleoid != NULL) {
      if (ber_printf(ber, "ts", LDAP_TAG_SK_MATCHRULE,
                     (sortKeyList[i])->sk_matchruleoid) == -1) {
        goto encoding_error_exit;
      }
    }
  ...
  rc = nsldapi_build_control(LDAP_CONTROL_SORTREQUEST, ber, 1, ctl_iscritical,
                             ctrlp);
...
int LDAP_CALL ldap_parse_sort_control(LDAP* ld, LDAPControl** ctrlp,
  ...
  /*  allocate a Ber element with the contents of the sort_control's struct
   * berval */
...
static int read_next_token(const char** s, LDAPsortkey** key) {
  ...
  while (((c = *pos++) != '\0') && (state != 4)) {
    switch (state) {
      ...
      case 1:
        ...
        break;
      case 2:
        /* case where we've seen the end of the attr and want the beginning of
         * match rule */
        if (!isspace(c)) {
        ...
        break;
      case 3:
        /* case where we've seen the beginning of match rule and want to find
         * the end */
        if (isspace(c)) {
  ...
  if (3 == state) {
    /* means we fell off the end of the string looking for the end of the
     * marching rule */
    matchrule_size = (pos - matchrule_source) - 1;
  }

  if (1 == state) {
    /* means we fell of the end of the string looking for the end of the
     * attribute */
    attrdesc_size = (pos - attrdesc_source) - 1;
  }
...
int LDAP_CALL ldap_create_sort_keylist(LDAPsortkey*** sortKeyList,
  ...
  /* Now walk along the string, allocating and filling in the LDAPsearchkey
   * structure */
  current_position = string_rep;

  for (i = 0; i < count; i++) {
    if (0 !=
        (retval = read_next_token(&current_position, &(pointer_array[i])))) {
