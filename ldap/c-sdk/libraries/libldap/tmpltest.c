...
int main(int argc, char** argv) {
  ...
  if ((err = ldap_init_templates("ldaptemplates.conf", &templates)) != 0) {
    fprintf(stderr, "ldap_init_templates failed (%d)\n", err);
    exit(1);
  }

  if ((err = ldap_init_searchprefs("ldapsearchprefs.conf", &so)) != 0) {
...
void dump_tmpl(struct ldap_disptmpl* tmpl) {
  ...
  if (tmpl->dt_options == 0L) {
    ...
    printf("%s %s %s\n",
           LDAP_IS_DISPTMPL_OPTION_SET(tmpl, LDAP_DTMPL_OPT_ADDABLE) ? "addable"
                                                                     : "",
           LDAP_IS_DISPTMPL_OPTION_SET(tmpl, LDAP_DTMPL_OPT_ALLOWMODRDN)
               ? "modrdn"
               : "",
           LDAP_IS_DISPTMPL_OPTION_SET(tmpl, LDAP_DTMPL_OPT_ALTVIEW) ? "altview"
                                                                     : "");
  }

  printf("authenticate as attribute: %s\n",
         tmpl->dt_authattrname != NULL ? tmpl->dt_authattrname : "<default>");

  printf("default RDN attribute:     %s\n",
         tmpl->dt_defrdnattrname != NULL ? tmpl->dt_defrdnattrname : "NONE");

  printf("default add location:      %s\n",
         tmpl->dt_defaddlocation != NULL ? tmpl->dt_defaddlocation : "NONE");
  ...
  if ((fetchattrs =
           ldap_tmplattrs(tmpl, includeattrs, 1, LDAP_SYN_OPT_DEFER)) == NULL) {
    printf("  <none>\n");
  } else {
  ...
  if ((fetchattrs = ldap_tmplattrs(tmpl, NULL, 0, LDAP_SYN_OPT_DEFER)) ==
      NULL) {
    printf("  <none>\n");
  } else {
    for (i = 0; fetchattrs[i] != NULL; ++i) {
  ...
  for (rowp = ldap_first_tmplrow(tmpl); rowp != NULLTMPLITEM;
    ...
    for (colp = ldap_first_tmplcol(tmpl, rowp); colp != NULLTMPLITEM;
      ...
      ++colcnt;
      printf(
          "  %2d-%d: %s (%s%s", rowcnt, colcnt,
          syn_name[colp->ti_syntaxid & 0x0000FFFF],
          syn_type[LDAP_GET_SYN_TYPE(colp->ti_syntaxid) >> 24],
          ((LDAP_GET_SYN_OPTIONS(colp->ti_syntaxid) & LDAP_SYN_OPT_DEFER) != 0)
              ? ",defer"
              : "");
