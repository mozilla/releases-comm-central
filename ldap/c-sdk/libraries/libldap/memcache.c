...
#define LDAP_MEMCACHE_MUTEX_ALLOC(c)              \
  ((c) && ((c)->ldmemc_lock_fns).ltf_mutex_alloc  \
       ? ((c)->ldmemc_lock_fns).ltf_mutex_alloc() \
       : NULL)
...
int LDAP_CALL ldap_memcache_init(unsigned long ttl, unsigned long size,
  ...
  if (thread_fns) {
    ...
    memset(&((*cachep)->ldmemc_lock_fns), 0, sizeof(struct ldap_thread_fns));
  }
  ...
  if (htable_create(size, attrkey_hashf, attrkey_putdata, attrkey_getdata,
                    attrkey_removedata, attrkey_clearnode, NULL,
                    &((*cachep)->ldmemc_resLookup)) != LDAP_SUCCESS) {
    ldap_memcache_destroy(*cachep);
    *cachep = NULL;
    return (LDAP_NO_MEMORY);
  }
...
int LDAP_CALL ldap_memcache_set(LDAP* ld, LDAPMemCache* cache) {
  ...
  if (ld->ld_memcache != cache) {
    ...
    if (pCur) {
      ...
      memcache_adj_size(c, sizeof(ldapmemcacheld), MEMCACHE_SIZE_NON_ENTRIES,
...
int ldap_memcache_createkey(LDAP* ld, const char* base, int scope,
  ...
  *keyp = 0;
  ...
  if ((binddn = nsldapi_get_binddn(ld)) == NULL) binddn = "";
  ...
  if (attrs) {
    for (i = 0; attrs[i]; i++) {
      for (i_smallest = j = i; attrs[j]; j++) {
        if (strcasecmp(attrs[i_smallest], attrs[j]) > 0) i_smallest = j;
      }
...
int ldap_memcache_append(LDAP* ld, int msgid, int bLast, LDAPMessage* result) {
  ...
  LDAPDebug(LDAP_DEBUG_TRACE, "ldap_memcache_append( ld: 0x%p, ", ld);
  ...
  LDAPDebug(LDAP_DEBUG_TRACE, "ldap_memcache_append: %s result for msgid %d\n",
            (nRes == LDAP_SUCCESS) ? "added" : "failed to add", msgid);
...
static void memcache_trim_basedn_spaces(char* basedn) {
  ...
  for (pWrite = pRead = basedn; *pRead;) {
    ...
    for (; *pRead && !NSLDAPI_IS_SEPARATER(*pRead); *(pWrite++) = *(pRead++)) {
      ;
    }
...
static int memcache_compare_dn(const char* main_dn, const char* dn, int scope) {
  ...
  if (!components || !main_components) {
    ...
    for (; i >= 0 && main_i >= 0; i--, main_i--) {
      if (strcasecmp(main_components[main_i], components[i])) break;
    }
  ...
  if (components) ldap_value_free(components);

  if (main_components) ldap_value_free(main_components);
...
static int memcache_dup_message(LDAPMessage* res, int msgid, int fromcache,
  ...
  *ppResCopy = NULL;
  ...
  for (pCur = res, ppCurNew = ppResCopy; pCur;
    ...
    if (pSize) *pSize += sizeof(LDAPMessage) + ber_size;
  }
...
static int memcache_free_entry(LDAPMemCache* cache, ldapmemcacheRes* pRes) {
  if (pRes) {
    ...
    memcache_adj_size(cache, size, MEMCACHE_SIZE_ENTRIES, MEMCACHE_SIZE_DEDUCT);
  }
...
static int memcache_free_from_list(LDAPMemCache* cache, ldapmemcacheRes* pRes,
  ...
  if (pRes->ldmemcr_next[index])
    pRes->ldmemcr_next[index]->ldmemcr_prev[index] = pRes->ldmemcr_prev[index];
...
static int memcache_add_res_to_list(ldapmemcacheRes* pRes, LDAPMessage* pMsg,
  ...
  for (pRes->ldmemcr_resTail = pMsg; pRes->ldmemcr_resTail->lm_chain;
...
static int memcache_expired(LDAPMemCache* cache, ldapmemcacheRes* pRes,
  ...
  return ((unsigned long)difftime((time_t)curTime,
...
static int memcache_access(LDAPMemCache* cache, int mode, void* pData1,
  ...
  /* Append entries to an existing cache header. */
    ...
    if (mode == MEMCACHE_ACCESS_APPEND) return (LDAP_SUCCESS);
  ...
  /* Remove cached entries in both primary and temporary cache. */
    ...
    if (cache->ldmemc_basedns) {
      ...
      if (cache->ldmemc_basedns[i] == NULL) return (LDAP_SUCCESS);
    }
    ...
    for (i = 0; i < 2; i++) {
      ...
      for (pRes = cache->ldmemc_resHead[list_id]; pRes != NULL;
        ...
        for (pMsg = pRes->ldmemcr_resHead, bDone = 0; !bDone && pMsg;
             pMsg = pMsg->lm_chain) {
          if (!NSLDAPI_IS_SEARCH_ENTRY(pMsg->lm_msgtype)) continue;
          foundentries = 1;
          ber = *(pMsg->lm_ber);
          if (ber_scanf(&ber, "{a", &dnTmp) != LBER_ERROR) {
            bDone =
                (memcache_compare_dn(dnTmp, dn, scope) == LDAP_COMPARE_TRUE);
            ldap_memfree(dnTmp);
          }
        }
...
static int htable_calculate_size(int sizelimit) {
  int i, j;
  int size =
      (int)(((double)sizelimit / (double)(sizeof(BerElement) + EXTRA_SIZE)) /
            1.5);
...
static int msgid_getdata(void* pTableData, void* key, void** ppData) {
  ...
  *ppData = NULL;
  ...
  if (!pCurRes) return (LDAP_NO_SUCH_OBJECT);
...
static int msgid_removedata(void** ppTableData, void* key, void** ppData) {
  ...
  if (!pHead) return (LDAP_NO_SUCH_OBJECT);
