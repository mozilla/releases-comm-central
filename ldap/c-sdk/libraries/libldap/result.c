...
int nsldapi_result_nolock(LDAP* ld, int msgid, int all, int unlock_permitted,
  ...
  if (check_response_queue(ld, msgid, all, 1, result) != 0) {
    ...
    rc = wait4msg(ld, msgid, all, unlock_permitted, timeout, result);
  }
...
static int wait4msg(LDAP* ld, int msgid, int all, int unlock_permitted,
  ...
  if (msgid != LDAP_RES_ANY && msgid != LDAP_RES_UNSOLICITED) {
    ...
    if ((lr = nsldapi_find_request_by_msgid(ld, msgid)) == NULL) {
    ...
    if (lr->lr_conn != NULL && lr->lr_conn->lconn_status == LDAP_CONNST_DEAD) {
  ...
  while (rc == NSLDAPI_RESULT_NOT_FOUND) {
    ...
    for (lc = ld->ld_conns; lc != NULL; lc = lc->lconn_next) {
      if (lc->lconn_sb->sb_ber.ber_ptr < lc->lconn_sb->sb_ber.ber_end) {
        /* read1msg() might free the connection. */
        rc = read1msg(ld, msgid, all, lc->lconn_sb, &lc, result);
    ...
    if (!msgfound) {
       ...
       * was EINTR, try again.
       */
      if (err == -1 && 0 != (ld->ld_options & LDAP_BITOPT_RESTART) &&
       ...
       */
      if (err == -1 || err == 0) {
        LDAP_SET_LDERRNO(ld, (err == -1 ? LDAP_SERVER_DOWN : LDAP_TIMEOUT),
                         NULL, NULL);
        if (err == -1) {
          LDAP_MUTEX_LOCK(ld, LDAP_REQ_LOCK);
      ...
      LDAP_MUTEX_LOCK(ld, LDAP_CONN_LOCK);
      LDAP_MUTEX_LOCK(ld, LDAP_REQ_LOCK);
      for (lc = ld->ld_conns; rc == NSLDAPI_RESULT_NOT_FOUND && lc != NULL;
           lc = nextlc) {
         ...
         */
        if (nsldapi_iostatus_is_read_ready(ld, lc->lconn_sb)) {
          /* read1msg() might free the connection. */
          rc = read1msg(ld, msgid, all, lc->lconn_sb, &lc, result);
        }
        ...
        if (lc && lc->lconn_pending_requests > 0 &&
            nsldapi_iostatus_is_write_ready(ld, lc->lconn_sb)) {
          err = nsldapi_send_pending_requests_nolock(ld, lc);
          if (err == -1 && rc == NSLDAPI_RESULT_NOT_FOUND) {
            rc = NSLDAPI_RESULT_ERROR;
          }
        }
      }
...
#define NSLDAPI_REQUEST_COMPLETE(lr)                                           \
  ((lr)->lr_outrefcnt <= 0 && (lr)->lr_res_msgtype != LDAP_RES_SEARCH_ENTRY && \
...
static int read1msg(LDAP* ld, int msgid, int all, Sockbuf* sb, LDAPConn** lcp,
  ...
  if (lc->lconn_ber == NULLBER &&
      nsldapi_alloc_ber_with_options(ld, &lc->lconn_ber) != LDAP_SUCCESS) {
  ...
  if ((tag = ber_get_next(sb, &len, lc->lconn_ber)) != LDAP_TAG_MESSAGE) {
    terrno = LDAP_GET_ERRNO(ld);
    if (terrno == EWOULDBLOCK || terrno == EAGAIN) {
      return (NSLDAPI_RESULT_NOT_FOUND); /* try again */
    }
    LDAP_SET_LDERRNO(
        ld, (tag == LBER_DEFAULT ? LDAP_SERVER_DOWN : LDAP_LOCAL_ERROR), NULL,
        NULL);
    if (tag == LBER_DEFAULT) {
  ...
  if (id != LDAP_RES_UNSOLICITED &&
    ...
    if (refchasing > 0 || lr->lr_outrefcnt > 0) {
       ...
       */
      if (!has_parent && LDAP_RES_BIND == lr->lr_res_msgtype &&
          lr->lr_conn != NULL) {
        if (lr->lr_conn->lconn_binddn != NULL) {
          NSLDAPI_FREE(lr->lr_conn->lconn_binddn);
        }
        if (LDAP_SUCCESS == nsldapi_parse_result(ld, lr->lr_res_msgtype, ber,
                                                 &lderr, NULL, NULL, NULL,
                                                 NULL) &&
            LDAP_SUCCESS == lderr) {
  ...
  for (l = ld->ld_responses; l != NULL; l = l->lm_next) {
    if (l->lm_msgid == new->lm_msgid) break;
    prev = l;
  }
   ...
   */
  if (manufactured_result && tmp->lm_msgtype == LDAP_RES_SEARCH_RESULT) {
    /*
...
static void check_for_refs(LDAP* ld, LDAPRequest* lr, BerElement* ber,
  ...
  *chasingcountp = *totalcountp = 0;
  ...
  if (lr->lr_res_msgtype == LDAP_RES_SEARCH_REFERENCE) {
    ...
    err = nsldapi_parse_result(ld, lr->lr_res_msgtype, ber, &origerr,
                               &matcheddn, &errstr, &v3refs, NULL);
  }
  ...
  if (origerr == LDAP_REFERRAL) { /* ldapv3 */
    if (v3refs != NULL) {
      err = nsldapi_chase_v3_refs(
          ld, lr, v3refs, (lr->lr_res_msgtype == LDAP_RES_SEARCH_REFERENCE),
          totalcountp, chasingcountp);
      ldap_value_free(v3refs);
    }
  } else if (ldapversion == LDAP_VERSION2 && origerr != LDAP_SUCCESS) {
    /* referrals may be present in the error string */
    err =
        nsldapi_chase_v2_referrals(ld, lr, &errstr, totalcountp, chasingcountp);
  }
  ...
  if (err == LDAP_SUCCESS && (*chasingcountp == *totalcountp)) {
    if (*totalcountp > 0 &&
        (origerr == LDAP_PARTIAL_RESULTS || origerr == LDAP_REFERRAL)) {
...
static int build_result_ber(LDAP* ld, BerElement** berp, LDAPRequest* lr) {
  ...
  *berp = ber;
  if (ber_printf(ber, lr->lr_res_ctrls ? "{it{ess}" : "{it{ess}}", lr->lr_msgid,
  ...
  if (NULL != lr->lr_res_ctrls &&
      nsldapi_put_controls(ld, lr->lr_res_ctrls, 1 /* close seq */, ber) !=
          LDAP_SUCCESS) {
    return (LDAP_ENCODING_ERROR);
  }
...
static int cldap_select1(LDAP* ld, struct timeval* timeout) {
  ...
  if (NSLDAPI_IO_TYPE_STANDARD == ld->ldiou_type && NULL != ld->ld_select_fn) {
...
static int ldap_mark_abandoned(LDAP* ld, int msgid) {
  ...
  for (i = 0; ld->ld_abandoned[i] != -1; i++)
    if (ld->ld_abandoned[i] == msgid) break;
...
int cldap_getmsg(LDAP* ld, struct timeval* timeout, BerElement** ber) {
  ...
  if (ld->ld_sbp->sb_ber.ber_ptr >= ld->ld_sbp->sb_ber.ber_end) {
    ...
    if (rc == -1 || rc == 0) {
      LDAP_SET_LDERRNO(ld, (rc == -1 ? LDAP_SERVER_DOWN : LDAP_TIMEOUT), NULL,
                       NULL);
      return (rc);
    }
  }
  ...
  /* get the next message */
  if ((tag = ber_get_next(ld->ld_sbp, &len, ber)) != LDAP_TAG_MESSAGE) {
    LDAP_SET_LDERRNO(
        ld, (tag == LBER_DEFAULT ? LDAP_SERVER_DOWN : LDAP_LOCAL_ERROR), NULL,
        NULL);
    return (-1);
