...
#  ifdef NSLDAPI_HAVE_POLL
  if ((n = poll(&pfd, 1, (msec != LDAP_X_IO_TIMEOUT_NO_TIMEOUT) ? msec : -1)) ==
      0) {
    errno = ETIMEDOUT;
    return (-1);
  }
...
#    ifdef _WINDOWS
  if ((n = select(sockfd + 1, &rset, &wset, &eset,
                  (msec != LDAP_X_IO_TIMEOUT_NO_TIMEOUT) ? &tval : NULL)) ==
      0) {
    errno = WSAETIMEDOUT;
    return (-1);
  }
 ...
 */
{
  ...
  if (NULL != ld->ld_extconnect_fn) {
    ...
    s = ld->ld_extconnect_fn(hostlist, defport, ld->ld_connect_timeout,
                             connect_opts, ld->ld_ext_session_arg,
    ...
    return (-1);
#else  /* NSLDAPI_AVOID_OS_SOCKETS */
    s = nsldapi_try_each_host(ld, hostlist, defport, secure, nsldapi_os_socket,
...
#  endif /* GETHOSTBYNAME_BUF_T */
  ...
  connected = 0;
  parse_err = ldap_x_hostlist_first(hostlist, defport, &host, &port, &status);
...
#  endif /* else GETHOSTBYNAME_buf_t */
        ...
        if ((ldhp = ld->ld_dns_gethostbyname_fn(
                 host, &ldhent, ldhpbuf, ld->ld_dns_bufsize, &err,
    ...
    for (i = 0; !use_hp || (addrlist[i] != 0); i++) {
      if (-1 == (s = (*socketfn)(ld, secure, AF_INET, SOCK_STREAM, 0))) {
        if (ldhpbuf_allocd != NULL) {
          NSLDAPI_FREE(ldhpbuf_allocd);
        }
        ldap_memfree(host);
...
void nsldapi_close_connection(LDAP* ld, Sockbuf* sb) {
  if (ld->ld_extclose_fn == NULL) {
    ...
    ld->ld_extclose_fn(sb->sb_sd, sb->sb_ext_io_fns.lbextiofn_socket_arg);
  }
}
...
int nsldapi_iostatus_interest_write(LDAP* ld, Sockbuf* sb) {
  ...
  if (ld->ld_iostatus == NULL && nsldapi_iostatus_init_nolock(ld) < 0) {
    rc = -1;
...
#else /* NSLDAPI_AVOID_OS_SOCKETS */
#  ifdef NSLDAPI_HAVE_POLL
    if (nsldapi_add_to_os_pollfds(sb->sb_sd, &iosp->ios_status.ios_osinfo,
                                  POLLOUT)) {
      ++iosp->ios_write_count;
    }
#  else  /* NSLDAPI_HAVE_POLL */
    if (!FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_writefds)) {
      FD_SET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_writefds);
      ++iosp->ios_write_count;
    }
#  endif /* else NSLDAPI_HAVE_POLL */
#endif   /* NSLDAPI_AVOID_OS_SOCKETS */
...
#  else  /* NSLDAPI_HAVE_POLL */
    if (!FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_readfds)) {
      FD_SET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_readfds);
      ++iosp->ios_read_count;
    }
#  endif /* else NSLDAPI_HAVE_POLL */
#endif   /* NSLDAPI_AVOID_OS_SOCKETS */
...
#else /* NSLDAPI_AVOID_OS_SOCKETS */
#  ifdef NSLDAPI_HAVE_POLL
    if (nsldapi_clear_from_os_pollfds(sb->sb_sd, &iosp->ios_status.ios_osinfo,
                                      POLLOUT)) {
      --iosp->ios_write_count;
    }
    if (nsldapi_clear_from_os_pollfds(sb->sb_sd, &iosp->ios_status.ios_osinfo,
...
#  else  /* NSLDAPI_HAVE_POLL */
    if (FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_writefds)) {
      FD_CLR(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_writefds);
      --iosp->ios_write_count;
    }
    if (FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_readfds)) {
      FD_CLR(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_readfds);
      --iosp->ios_read_count;
    }
#  endif /* else NSLDAPI_HAVE_POLL */
#endif   /* NSLDAPI_AVOID_OS_SOCKETS */
...
#  ifdef NSLDAPI_HAVE_POLL
     ...
     */
    rc = nsldapi_find_in_os_pollfds(sb->sb_sd, &iosp->ios_status.ios_osinfo,
                                    ~POLLIN);

#  else  /* NSLDAPI_HAVE_POLL */
    rc = FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_use_writefds);
#  endif /* else NSLDAPI_HAVE_POLL */
#endif   /* NSLDAPI_AVOID_OS_SOCKETS */
...
#  else  /* NSLDAPI_HAVE_POLL */
    rc = FD_ISSET(sb->sb_sd, &iosp->ios_status.ios_osinfo.ossi_use_readfds);
#  endif /* else NSLDAPI_HAVE_POLL */
#endif   /* NSLDAPI_AVOID_OS_SOCKETS */
...
void nsldapi_iostatus_free(LDAP* ld) {
  ...
  if (ld->ld_iostatus != NULL) {
    ...
    if (iosp->ios_type == NSLDAPI_IOSTATUS_TYPE_OSNATIVE) {
#ifdef NSLDAPI_HAVE_POLL
      if (iosp->ios_status.ios_osinfo.ossi_pollfds != NULL) {
        NSLDAPI_FREE(iosp->ios_status.ios_osinfo.ossi_pollfds);
      }
#endif /* NSLDAPI_HAVE_POLL */
...
#    else
    rc = NSLDAPI_SELECT(nsldapi_get_select_table_size(),
                        &iosp->ios_status.ios_osinfo.ossi_use_readfds,
                        &iosp->ios_status.ios_osinfo.ossi_use_writefds, NULL,
                        timeout);
#    endif /* else HPUX9 */
#  endif   /* else NSLDAPI_HAVE_POLL */
#endif     /* NSLDAPI_AVOID_OS_SOCKETS */
...
static int nsldapi_add_to_os_pollfds(int fd, struct nsldapi_os_statusinfo* pip,
  ...
  for (i = 0; i < pip->ossi_pollfds_size; ++i) {
    if (pip->ossi_pollfds[i].fd == fd) {
      if ((pip->ossi_pollfds[i].events & events) != events) {
  ...
  if (openslot == -1) {
    ...
    for (i = openslot + 1; i < pip->ossi_pollfds_size; ++i) {
      ...
      pip->ossi_pollfds[i].events = pip->ossi_pollfds[i].revents = 0;
    }
  }
...
static int nsldapi_add_to_cb_pollfds(Sockbuf* sb,
  ...
  for (i = 0; i < pip->cbsi_pollfds_size; ++i) {
    if (NSLDAPI_CB_POLL_MATCH(sb, pip->cbsi_pollfds[i])) {
      if ((pip->cbsi_pollfds[i].lpoll_events & events) != events) {
  ...
  if (openslot == -1) {
    ...
    for (i = openslot + 1; i < pip->cbsi_pollfds_size; ++i) {
      ...
      pip->cbsi_pollfds[i].lpoll_events = pip->cbsi_pollfds[i].lpoll_revents =
          0;
    }
  }
...
static int nsldapi_clear_from_cb_pollfds(Sockbuf* sb,
  ...
  for (i = 0; i < pip->cbsi_pollfds_size; ++i) {
    if (NSLDAPI_CB_POLL_MATCH(sb, pip->cbsi_pollfds[i])) {
      if ((pip->cbsi_pollfds[i].lpoll_events & events) != 0) {
        pip->cbsi_pollfds[i].lpoll_events &= ~events;
        if (pip->cbsi_pollfds[i].lpoll_events == 0) {
          pip->cbsi_pollfds[i].lpoll_fd = -1;
        }
        return (1); /* events overlap */
      } else {
...
static int nsldapi_find_in_cb_pollfds(Sockbuf* sb,
  ...
  for (i = 0; i < pip->cbsi_pollfds_size; ++i) {
    if (NSLDAPI_CB_POLL_MATCH(sb, pip->cbsi_pollfds[i])) {
      if ((pip->cbsi_pollfds[i].lpoll_revents & revents) != 0) {
        return (1); /* revents overlap */
      } else {
        return (0); /* revents do not overlap */
      }
...
int nsldapi_install_lber_extiofns(LDAP* ld, Sockbuf* sb) {
  ...
  if (NULL != sb) {
    ...
    if (ber_sockbuf_set_option(sb, LBER_SOCKBUF_OPT_EXT_IO_FNS, &lberiofns) !=
        0) {
...
static LBER_SOCKET nsldapi_compat_socket(LDAP* ld, int secure, int domain,
  ...
  if (s >= 0) {
    ...
    if (NULL == errmsg && secure && ld->ld_io_fns_ptr->liof_ssl_enable(s) < 0) {
    ...
    if (NULL != errmsg) {
      ...
      LDAP_SET_LDERRNO(ld, LDAP_LOCAL_ERROR, NULL, nsldapi_strdup(errmsg));
      return (-1);
    }
  }
...
static int LDAP_CALLBACK nsldapi_ext_compat_connect(
  ...
  closefn =
      (iofns->liof_close == NULL) ? nsldapi_os_closesocket : iofns->liof_close;

  s = nsldapi_try_each_host(defcsip->csi_ld, hostlist, defport, secure,
                            socketfn, ioctlfn, connectwithtofn, connectfn,
                            closefn);
