...
static int skip_url_prefix(const char** urlp, int* enclosedp, int* securep) {
  ...
  if (strlen(*urlp) >= LDAP_URL_URLCOLON_LEN &&
      strncasecmp(*urlp, LDAP_URL_URLCOLON, LDAP_URL_URLCOLON_LEN) == 0) {
  ...
  if (strlen(*urlp) >= LDAPS_URL_PREFIX_LEN &&
      strncasecmp(*urlp, LDAPS_URL_PREFIX, LDAPS_URL_PREFIX_LEN) == 0) {
    /* skip over URL prefix and return success */
...
int LDAP_CALL ldap_url_parse_no_defaults(const char* url, LDAPURLDesc** ludpp,
                                         int dn_required) {
...
int nsldapi_url_parse(const char* url, LDAPURLDesc** ludpp, int dn_required) {
  ...
  *ludpp = NULL; /* pessimistic */
  ...
  attrs = NULL;
  if (ludp->lud_dn != NULL && (attrs = strchr(ludp->lud_dn, '?')) != NULL) {
      ...
      if ((p = strchr(scope, '?')) != NULL) {
        ...
        *p++ = '\0';
        if (*p != '\0') {
           ...
           * extensions
           */
          if ((p = strchr(ludp->lud_filter, '?')) != NULL) {
            *p++ = '\0'; /* term. filter */
            extensions = p;
          }
  ...
  if (attrs != NULL && *attrs != '\0') {
    ...
    if ((ludp->lud_attrs = (char**)NSLDAPI_CALLOC(nattrs + 1, sizeof(char*))) ==
        NULL) {
      ldap_free_urldesc(ludp);
      return (LDAP_URL_ERR_MEM);
    }
...
void LDAP_CALL ldap_free_urldesc(LDAPURLDesc* ludp) {
...
int LDAP_CALL ldap_url_search(LDAP* ld, const char* url, int attrsonly) {
  int err, msgid;
  LDAPURLDesc* ludp;
  BerElement* ber;
  LDAPServer* srv;
  ...
  if (nsldapi_build_search_req(
          ld, ludp->lud_dn, ludp->lud_scope, ludp->lud_filter, ludp->lud_attrs,
          attrsonly, NULL, NULL, -1, -1, msgid, &ber) != LDAP_SUCCESS) {
    return (-1);
  }
  ...
  if ((srv = (LDAPServer*)NSLDAPI_CALLOC(1, sizeof(LDAPServer))) == NULL ||
      (host != NULL && (srv->lsrv_host = nsldapi_strdup(host)) == NULL)) {
    if (srv != NULL) {
      NSLDAPI_FREE(srv);
    }
  ...
  if (err != 0) {
    ...
    err = nsldapi_send_server_request(ld, ber, msgid, NULL, srv, NULL, NULL, 1);
  }
...
int LDAP_CALL ldap_url_search_st(LDAP* ld, const char* url, int attrsonly,
...
int LDAP_CALL ldap_url_search_s(LDAP* ld, const char* url, int attrsonly,
                                LDAPMessage** res) {
  int msgid;
