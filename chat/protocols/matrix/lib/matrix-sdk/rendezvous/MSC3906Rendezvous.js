...
class MSC3906Rendezvous {
  ...
  async startAfterShowingCode() {
    ...
    if (!capability?.enabled && features.get(_feature.Feature.LoginTokenRequest) === _feature.ServerSupport.Unsupported) {
      ...
      await this.cancel(_.LegacyRendezvousFailureReason.HomeserverLacksSupport);
      return undefined;
    }
    await this.send({
    ...
    if (type === PayloadType.Finish) {
      ...
      switch (outcome ?? "") {
        case "unsupported":
          await this.cancel(_.LegacyRendezvousFailureReason.UnsupportedAlgorithm);
          break;
        default:
          await this.cancel(_.LegacyRendezvousFailureReason.Unknown);
      }
      return undefined;
    }
