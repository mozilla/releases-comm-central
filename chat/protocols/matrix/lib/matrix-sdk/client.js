...
var _readReceipt = require("./models/read-receipt");
var _invitesIgnorer = require("./models/invites-ignorer");
var _sync2 = require("./@types/sync");
...
class MatrixClient extends _typedEventEmitter.TypedEventEmitter {
  ...
  getVisibleRooms() {
    ...
    for (const r of allRooms) {
      const createEvent = r.currentState.getStateEvents(_event2.EventType.RoomCreate, '');
  ...
  async joinRoom(roomIdOrAlias, opts = {}) {
    ...
    if (opts.inviteSignUrl) {
      signPromise = this.http.requestOtherUrl(_httpApi.Method.Post, new URL(opts.inviteSignUrl), {
  ...
  sendEvent(roomId, threadId, eventType, content, txnId) {
   ...
   * @return {module:http-api.MatrixError} Rejects: with an error response.
   */
  async sendReceipt(event, receiptType, body) {
    if (this.isGuest()) {
    ...
    if (await this.doesServerSupportUnstableFeature("org.matrix.msc3771")) {
      ...
      body.thread_id = isThread ? event.threadRootId : _readReceipt.MAIN_ROOM_TIMELINE;
    }
   ...
   * @return {module:http-api.MatrixError} Rejects: with an error response.
   */
  async sendReadReceipt(event, receiptType = _read_receipts.ReceiptType.Read) {
    if (!event) return;
  ...
  paginateEventTimeline(eventTimeline, opts) {
    ...
    if (isNotifTimeline) {
      ...
      promise = this.createMessagesRequest(eventTimeline.getRoomId(), token, opts.limit, dir, eventTimeline.getFilter()).then(res => {
        ...
        this.processBeaconEvents(room, timelineEvents);
        this.processThreadRoots(room, timelineEvents.filter(it => it.isRelation(_thread.THREAD_RELATION_TYPE.name)), false);
  ...
  async getVersions() {
    ...
    const support = this.canSupport.get(_feature.Feature.ThreadUnreadNotifications);
    _sync2.UNREAD_THREAD_NOTIFICATIONS.setPreferUnstable(support === _feature.ServerSupport.Unstable);
    return this.serverVersionsPromise;
  }
  ...
  * @return {Object} the response, with chunk, prev_batch and, next_batch.
  ...
  search(opts // eslint-disable-line camelcase
    ...
    return this.http.authedRequest(_httpApi.Method.Post, "/search", queryParams, opts.body);
  }
  ...
  submitMsisdnTokenOtherUrl(url, sid, clientSecret, msisdnToken) {
    ...
    return this.http.requestOtherUrl(_httpApi.Method.Post, u);
  }
...
function fixNotificationCountOnDecryption(cli, event) {
  ...
  const currentCount = (isThreadEvent ? room.getThreadUnreadNotificationCount(event.threadRootId, _matrix.NotificationCountType.Highlight) : room.getUnreadNotificationCount(_matrix.NotificationCountType.Highlight)) ?? 0;
  ...
  if (oldHighlight !== newHighlight || currentCount > 0) {
    ...
    const hasReadEvent = isThreadEvent ? room.getThread(event.threadRootId)?.hasUserReadEvent(cli.getUserId(), event.getId()) : room.hasUserReadEvent(cli.getUserId(), event.getId());
    if (!hasReadEvent) {
      let newCount = currentCount;
      if (newHighlight && !oldHighlight) newCount++;
      if (!newHighlight && oldHighlight) newCount--;
      if (isThreadEvent) {
        room.setThreadUnreadNotificationCount(event.threadRootId, _matrix.NotificationCountType.Highlight, newCount);
      } else {
        room.setUnreadNotificationCount(_matrix.NotificationCountType.Highlight, newCount);
      }

      // Fix 'Mentions Only' rooms from not having the right badge count
      const totalCount = (isThreadEvent ? room.getThreadUnreadNotificationCount(event.threadRootId, _matrix.NotificationCountType.Total) : room.getUnreadNotificationCount(_matrix.NotificationCountType.Total)) ?? 0;
      if (totalCount < newCount) {
        if (isThreadEvent) {
          room.setThreadUnreadNotificationCount(event.threadRootId, _matrix.NotificationCountType.Total, newCount);
        } else {
