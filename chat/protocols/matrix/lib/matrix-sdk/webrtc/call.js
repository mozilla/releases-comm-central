...
class MatrixCall extends _typedEventEmitter.TypedEventEmitter {
  ...
  constructor(opts) {
    ...
    _defineProperty(this, "gotLocalOffer", async description => {
      ...
      content[_callEventTypes.SDPStreamMetadataKey] = this.getLocalSDPStreamMetadata();
    ...
    _defineProperty(this, "onIceConnectionStateChanged", () => {
      ...
      _logger.logger.debug("Call ID " + this.callId + ": ICE connection state changed to: " + this.peerConn?.iceConnectionState);
  ...
  gotCallFeedsForInvite(callFeeds) {
    ...
    _logger.logger.debug("gotUserMediaForInvite");
  ...
  async onSelectAnswerReceived(event) {
    ...
    if (selectedPartyId !== this.ourPartyId) {
      _logger.logger.info(`Got select_answer for party ID ${selectedPartyId}: we are party ID ${this.ourPartyId}.`);
