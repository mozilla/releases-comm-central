...
class MegolmEncryption extends _base.EncryptionAlgorithm {
  ...
  constructor(params) {
    ...
    _defineProperty(this, "setupPromise", Promise.resolve(undefined));
    ...
    _defineProperty(this, "encryptionPreparationMetadata", void 0);
  ...
  async ensureOutboundSession(room, devicesInRoom, blocked, singleOlmCreationPhase = false) {
    ...
    const prepareSession = async oldSession => {
      ...
      if (session && session.needsRotation(this.sessionRotationPeriodMsgs, this.sessionRotationPeriodMs)) {
      ...
      await Promise.all([(async () => {
        ...
        _logger.logger.debug(`Sharing keys with existing Olm sessions in ${this.roomId}`);
        ...
        _logger.logger.debug(`Sharing keys (start phase 1) with new Olm sessions in ${this.roomId}`);
        ...
        _logger.logger.debug(`Notifying blocked devices in ${this.roomId}`); // also, notify blocked devices that they're blocked
        ...
        _logger.logger.debug(`Notified ${blockedCount} blocked devices in ${this.roomId}`);
      })()]);
    ...
    const prom = this.setupPromise.then(prepareSession); // Ensure any failures are logged for debugging
    ...
    this.setupPromise = prom.then(returnSession, returnSession); // but we return a promise which only resolves if the share was successful.
  ...
  encryptAndSendKeysToDevices(session, chainIndex, userDeviceMap, payload) {
    ...
    return Promise.all(promises).then(() => {
      ...
      return this.baseApis.sendToDevice("m.room.encrypted", contentMap).then(() => {
        ...
        for (const userId of Object.keys(contentMap)) {
          for (const deviceId of Object.keys(contentMap[userId])) {
            session.markSharedWithDevice(userId, deviceId, deviceInfoByDeviceId.get(deviceId).getIdentityKey(), chainIndex);
          }
        }
      });
    });
  ...
  async sendBlockedNotificationsToDevices(session, userDeviceMap, payload) {
    ...
    await this.baseApis.sendToDevice("org.matrix.room_key.withheld", contentMap); // store that we successfully uploaded the keys of the current slice
  ...
  async shareKeyWithDevices(session, key, payload, devicesByUser, errorDevices, otkTimeout, failedServers) {
    ...
    _logger.logger.debug(`Sharing keys with Olm sessions in ${this.roomId}`);

    await this.shareKeyWithOlmSessions(session, key, payload, devicemap);

    _logger.logger.debug(`Shared keys with Olm sessions in ${this.roomId}`);
  }
  ...
  async notifyFailedOlmDevices(session, key, failedDevices) {
    ...
    const filteredFailedDevices = await this.olmDevice.filterOutNotifiedErrorDevices(failedDevices);

    _logger.logger.debug(`Filtered down to ${filteredFailedDevices.length} error devices ` + `in ${this.roomId}`);
    ...
    _logger.logger.debug(`Notified ${filteredFailedDevices.length} devices we failed to ` + `create Olm sessions in ${this.roomId}`);
  }
  /**
...
class MegolmDecryption extends _base.DecryptionAlgorithm {
  ...
  onRoomKeyEvent(event) {
    ...
    return this.olmDevice.addInboundGroupSession(content.room_id, senderKey, forwardingKeyChain, sessionId, content.session_key, keysClaimed, exportFormat, extraSessionData).then(() => {
      // have another go at decrypting events sent with this session.
      this.retryDecryption(senderKey, sessionId).then(success => {
